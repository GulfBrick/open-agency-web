...
export async function GET(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    const res = await fetch(`${agencyUrl}/api/clients/${id}/tasks`, {
      headers: {
    ...
    if (!res.ok) {
      console.error(`[clients/${id}/tasks proxy] backend error ${res.status}`)
    ...
    console.error(`[clients/${id}/tasks proxy] fetch failed:`, err)
    return NextResponse.json([])
  }
}
