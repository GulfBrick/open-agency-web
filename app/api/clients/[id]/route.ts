...
export async function GET(req: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    const res = await fetch(`${agencyUrl}/api/clients/${id}`, {
      headers: {
    ...
    if (!res.ok) {
      console.error(`[clients/${id} proxy] backend error ${res.status}`)
      return NextResponse.json({ error: `Backend error: ${res.status}` }, { status: 503 })
    }

    const data = await res.json()
    return NextResponse.json(data)
  } catch (err) {
    console.error(`[clients/${id} proxy] fetch failed:`, err)
