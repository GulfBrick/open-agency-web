...
export async function POST(req: NextRequest) {
  ...
  if (!body.clientId || !body.agentId) {
    return NextResponse.json({ error: 'clientId and agentId required' }, { status: 400 })
  }

  try {
    const res = await fetch(`${AGENCY_URL}/api/tasks/trigger`, {
      method: 'POST',
